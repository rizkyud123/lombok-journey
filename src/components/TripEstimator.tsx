...
import { Calculator, MessageCircle, Sparkles, Check, Send, Users, Calendar, MapPin } from 'lucide-react';
import { BUSINESS_INFO } from '../data/travelData';

export const TripEstimator: React.FC = () => {
  const [tripType, setTripType] = useState<string>('Private Trip');
  const [paxCount, setPaxCount] = useState<string>('3-5 Orang');
  const [duration, setDuration] = useState<string>('3 Hari 2 Malam');
  const [selectedSpots, setSelectedSpots] = useState<string[]>([
    'Gili Trawangan & Meno',
    'Kuta Mandalika & Bukit Merese'
  ]);

  const availableSpots = [
    'Gili Trawangan & Meno',
    'Kuta Mandalika & Sirkuit',
    'Bukit Merese & Tanjung Aan',
    'Air Terjun Tiu Kelep & Sendang Gile',
    'Desa Adat Sade & Sukarara',
    'Lembah Sembalun & Rinjani View',
    'Pink Beach Lombok'
  ];
  ...
  const generatedText = `Halo Lombok Journey! Saya ingin pesan / konsultasi penawaran trip dengan detail berikut:
• Tipe Trip: ${tripType}
• Jumlah Peserta: ${paxCount}
• Durasi Liburan: ${duration}
• Destinasi Pilihan: ${selectedSpots.length > 0 ? selectedSpots.join(', ') : 'Belum ditentukan'}

Mohon informasi rincian estimasi biaya & pilihan kendaraan yang tersedia. Terima kasih!`;

  const waUrl = `${BUSINESS_INFO.waBaseUrl}?text=${encodeURIComponent(generatedText)}`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <p className="text-base sm:text-lg text-slate-300 font-light">
            Pilih preferensi liburan Anda di bawah ini dan dapatkan estimasi penawaran resmi dari tim Lombok Journey via WhatsApp dalam hitungan menit!
          </p>
        </div>
        ...
        <div className="mt-12 bg-white text-slate-800 rounded-3xl p-6 sm:p-10 shadow-2xl border border-white/20 grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div>
              <label className="block text-xs font-bold text-[#112D4E] uppercase tracking-wider mb-2">
                1. Pilih Jenis Paket Trip
              </label>
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                {['Private Trip', 'Full Day Trip', 'Short Trip'].map((type) => (
                  <button
                    key={type}
                    type="button"
                    onClick={() => setTripType(type)}
                    className={`py-3 px-3 rounded-2xl text-xs sm:text-sm font-semibold border transition-all ${
                      tripType === type
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                >
                  <option value="1 - 2 Orang (Pasangan/Honeymoon)">1 - 2 Orang (Pasangan)</option>
                  <option value="3 - 5 Orang (Keluarga/Sahabat)">3 - 5 Orang (Keluarga)</option>
                  <option value="6 - 10 Orang (Grup Sedang)">6 - 10 Orang (Grup)</option>
                  <option value="10+ Orang (Rombongan Besar)">10+ Orang (Rombongan)</option>
                </select>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[#F5F5F5] border border-slate-200 rounded-xl py-3 px-3 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#112D4E]"
                >
                  <option value="1 Hari (Full Day)">1 Hari (Full Day Trip)</option>
                  ...
                  <option value="Custom Duration">Custom (Tentukan Sendiri)</option>
                </select>
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-[#112D4E] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                ...
                <span>4. Pilih Spot Wisata Favorit (Bisa pilih lebih dari satu)</span>
              </label>
          ...
          {/* Realtime Output & Direct WA Action */}
          <div className="lg:col-span-5 bg-[#F5F5F5] rounded-2xl p-6 border border-slate-200 flex flex-col justify-between space-y-6">
            <div>
              ...
              <div className="mt-4 space-y-3 text-xs text-slate-700">
                <div className="flex justify-between py-1 border-b border-slate-200/60">
                  <span className="text-slate-500">Tipe Trip:</span>
            ...
            <div className="space-y-3 pt-4 border-t border-slate-200">
              <a
                ...
                className="w-full flex items-center justify-center gap-2 bg-[#D4AF37] hover:bg-[#b8952b] text-[#112D4E] font-extrabold text-sm py-4 rounded-xl shadow-lg hover:shadow-xl transition-all"
              >
                <MessageCircle className="w-5 h-5 fill-current" />
                <span>Kirim Ke WhatsApp (0888-9163-745)</span>
              </a>

              <p className="text-[11px] text-slate-500 text-center font-light">
                ⚡ Respon cepat admin Lombok Journey & Konsultasi 100% Gratis.
              </p>
            </div>
